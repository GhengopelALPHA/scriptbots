...
namespace conf {
	...
	const int ENOUGHBOTS= 100; //number of bots where we no longer seed with random spawns
	const int TOOMANYBOTS= 1500; //number of bots at which the full NOAIR healthloss is applied
	...
	const int FRESHKILLTIME= 10; //number of ticks after a spike, collision, bite that a bot will still drop full meat
	...
	const float REPRATE=25; //amount of food required to be consumed for an agent to reproduce
	const float LEARNRATE= 0.001; // 0.02 (high-gen feedback)//0.0005 (pre-alt) //how quickly a conn weight can change from use
	...
	const float METAMUTRATE2= 0.001;
	const int MAXAGE=1000; //Age at which the full HEALTHLOSS_AGING amount is applied to an agent

	//distances
	const float DIST= 300; //how far can the senses can detect other bots or cells
	...
	const float HEALTHLOSS_WHEELS = 0.00001; //0.0 //How much health is lost for a bot driving at full speed
	const float HEALTHLOSS_BOOSTMULT=4; //how much boost costs (set to 1 to nullify boost cost; its a multiplier)
	const float HEALTHLOSS_BADTEMP = 0.0025; //how quickly does health drain in nonpreferred temperatures (0= disabled. 0.003 is decent value)
	const float HEALTHLOSS_AGING = 0.0001;
	const float HEALTHLOSS_BRAINUSE= 0.0001; //0.001 //how much health is reduced for each box in the brain being active
	...
	const float HEALTHLOSS_NOOXYGEN= 0.0008; //how much bots are penalized when total agents = TOOMANYBOTS, applied exponentially
	...
	const float FOODGROWTH= 0.0000052; //how much does food increase by on a cell with both plant and hazard? (fertilizer)
	...
	const float FOODSPREAD= 0.0002; //0.00015 //probability of a food cell seeding food to a nearby cell when it has high plant life
	...
	const int FRUITADDFREQ = 6; //how often does a high-plant-food cell get set to full fruit?
	...
	const float HAZARDDAMAGE= 0.020; //how much health a bot looses while on a filled hazard cell per tick?
